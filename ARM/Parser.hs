module ARM.Parser (parseARM) where
   ...
   parseARM program =
      case parse (many1 armParser) "" program of
         Left e -> error $ show e
         Right is -> is

   armReserved = 
      let 
         dpOps, bOps, pseudoOps, memOps, ccs, regs, shifts, flags :: [String]
         dpOps = map show ([minBound..maxBound] :: [DPOpcode])
         ccs = map show ([minBound..maxBound] :: [ConditionCode]) ++ 
               ["HS", "LO"]
            ...
            memOffset = 
               ...
               (do dir <- option UP ((symbol "-" >> return DOWN) <|>
                   ...
                   case so of
                     ...
                     _              -> error $ "Invalid shift for memory op " ++
                                               show so)
               <|>
               return (UP, MEMI 0)
   ...
   shifterOperand = (do rm <- register
                        option (SI rm LSL 0) $ do
                           sh <- shiftOp
                           liftM (SR rm sh) register <|> 
                              liftM (SI rm sh) constant)
